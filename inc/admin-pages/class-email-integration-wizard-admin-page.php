<?php
/**
 * Email Provider Integration Wizard Admin Page.
 *
 * @package WP_Ultimo
 ...
 * @since 2.3.0
 */
class Email_Integration_Wizard_Admin_Page extends Wizard_Admin_Page {
	 ...
	 * @var string
	 */
	protected $id = 'wp-ultimo-email-integration-wizard';
	 ...
	 *
	 * @since 2.3.0
	 * @var \WP_Ultimo\Integrations\Email_Providers\Base_Email_Provider
	 */
	...
	public function page_loaded(): void {
		...
		if (isset($_GET['integration'])) { // phpcs:ignore WordPress.Security.NonceVerification
			$email_manager = \WP_Ultimo\Managers\Email_Account_Manager::get_instance();

			$this->integration = $email_manager->get_provider_instance(sanitize_text_field(wp_unslash($_GET['integration']))); // phpcs:ignore WordPress.Security.NonceVerification
		}

		if ( ! $this->integration) {
			wp_safe_redirect(network_admin_url('admin.php?page=wp-ultimo-settings&tab=email-accounts'));
	...
	public function get_title(): string {
		...
		return __('Email Provider Setup', 'ultimate-multisite');
	}
	...
	public function get_menu_title() {
		...
		return __('Email Provider Integration', 'ultimate-multisite');
	}
	...
	public function get_sections() {
		...
		/*
		 * Some providers require no instructions.
		 */
		if (method_exists($this->integration, 'supports') && $this->integration->supports('no-instructions')) {
			unset($sections['instructions']);
		}

		/*
		 * Some providers require no additional setup.
		 */
		if (method_exists($this->integration, 'supports') && $this->integration->supports('no-config')) {
	...
	public function section_activation(): void {
		...
		wu_get_template(
			'wizards/email-integrations/activation',
			[
	...
	public function section_instructions(): void {
		...
		if (method_exists($this->integration, 'get_instructions')) {
			...
			wu_get_template(
				'wizards/email-integrations/default-instructions',
				[
					'screen'      => get_current_screen(),
					'page'        => $this,
					'integration' => $this->integration,
	...
	public function section_configuration(): void {
		...
		if (wu_request('manual')) {
			wu_get_template(
				'wizards/email-integrations/configuration-results',
				[
					'screen'      => get_current_screen(),
					'page'        => $this,
					'integration' => $this->integration,
		...
		wu_get_template(
			'wizards/email-integrations/configuration',
			[
				'screen'      => get_current_screen(),
				'page'        => $this,
				'integration' => $this->integration,
	...
	public function section_ready(): void {
		...
		wu_get_template(
			'wizards/email-integrations/ready',
			[
				'screen'      => get_current_screen(),
				'page'        => $this,
				'integration' => $this->integration,
	...
	public function section_test(): void {
		...
		wu_get_template(
			'wizards/email-integrations/test',
			[
				'screen'      => get_current_screen(),
				'page'        => $this,
				'integration' => $this->integration,
	...
	public function register_scripts() {
		...
		wp_enqueue_script(
			'wu-email-integration-test',
			wu_get_asset('email-integration-test.js', 'js'),
			[
				'jquery',
				'wu-vue',
			],
		...
		wp_add_inline_script(
			'wu-email-integration-test',
			'var wu_email_integration_test_data = {
